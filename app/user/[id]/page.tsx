  ...
  <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.8)", zIndex: 1000, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
    ...
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: "2rem", marginTop: "1rem" }}>
      {showImageUrl.length > 1 && (
        <button onClick={() => setCurrentImageIndex((currentImageIndex - 1 + showImageUrl.length) % showImageUrl.length)} style={{ background: "transparent", color: "white", fontSize: "2rem", border: "none", cursor: "pointer" }}>
          ❮
        </button>
      )}
      <button onClick={() => setShowImageUrl(null)} style={{ background: "white", border: "none", borderRadius: "8px", padding: "0.5rem 1rem", fontWeight: "bold" }}>
        Zavřít
      </button>
      {showImageUrl.length > 1 && (
        <button onClick={() => setCurrentImageIndex((currentImageIndex + 1) % showImageUrl.length)} style={{ background: "transparent", color: "white", fontSize: "2rem", border: "none", cursor: "pointer" }}>
          ❯
        </button>
      )}
    </div>
